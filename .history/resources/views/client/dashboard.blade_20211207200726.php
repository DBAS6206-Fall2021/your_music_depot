<div>
   
    <div class="card">
        <div class="card-header text-center">Client Dashboard</div>
    </div>

    <div class="bg-white d-flex m-0 p-0">
        <div class="row justify-content-center">
            <div class="col-sm">
                <div class="col-sm">
                    <h1 class="h3 mt-3 mb-3 font-weight-normal text-center">
                        Your Students
                    </h1>
                </div>
            </div>
            <div class="row">
                <div class="col-sm">
                    <table class="table table-sm table-striped">
                        <tbody>
                            <tr class="text-center">
                                <th class="col-sm-5">Name</th>
                                <th>Manage</th>
                            </tr>
                            <tr class="text-center">
                                <td>John Smith</td>
                                <td class="d-flex justify-content-between">
                                    <div>
                                        <a href="#" class="btn btn-primary btn-block">Edit</a>
                                    </div>
                                    <div>
                                        <a href="#" class="btn btn-primary btn-block">Lessons</a>
                                    </div>
                                    <div>
                                        <a href="" class="btn btn-primary btn-block">Book</a>
                                    </div>
                                    <div>
                                        <a href="/student/{{$students->first()->id}}/lessons/create" class="btn btn-primary btn-block">Remove</a>
                                    </div>
                                </td>
                            </tr>
                        </tbody>
                    </table>
                    <div class="d-flex justify-content-center">
                        <a href="#" class="btn btn-primary btn-block col-sm-5">
                            Add New Student
                        </a>
                    </div>
                </div>
            </div>
        </div>
        <div class="col-sm">
            <div class="row">
                <div class="col-sm">
                    <h1 class="h3 mt-3 mb-3 font-weight-normal text-center">
                        Upcoming Lessons
                    </h1>
                </div>
            </div>
            <div class="row">
                <div class="col-sm">
                    <table class="table table-sm table-striped">
                        <tbody>
                            <tr  class="text-center">
                                <th>Student</th>
                                <th>Lesson Type</th>
                                <th>Instructor</th>
                                <th>Date</th>
                                <th>Starts</th>
                                <th>Ends</th>
                                <th>Cancel Lessons</th>
                            </tr>
                            <tr class="text-center">
                                <td>John Smith</td>
                                <td>Piano</td>
                                <td>James Hancock</td>
                                <td>2021-01-01</td>
                                <td>4:00 PM</td>
                                <td>5:00 PM</td>
                                <td></tdclass>
                                    <a href="#" class="btn btn-primary btn-block">Cancel</a>
                                </td>
                            </tr>
                        </tbody>
                    </table>
                </div>
            </div>
        </div>
    </div>
</div>