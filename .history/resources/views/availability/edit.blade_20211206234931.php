            ...
            <tr class="text-center">
                ...
                <td>
                    ...
                    <select id="{{$day}}_start" name="{{$day}}_start">
                        ...
                        @for ($i = 8; $i <= 20; $i++)
                            <option value="{{$i}}:00:00" {{ $availability->firstWhere('weekday', $day)->hasAvailability("$i:00:00") ? "selected" : null}}>{{$i}}:00:00</option>
                            <option value="{{$i}}:00:00" {{ $availability->where('weekday', $day)->each(function ($item, $key) {$item->hasAvailability("$i:00:00") ? "selected" : null;}) }}>{{$i}}:00:00</option>
                        @endfor    
                    </select>    
                </td>
